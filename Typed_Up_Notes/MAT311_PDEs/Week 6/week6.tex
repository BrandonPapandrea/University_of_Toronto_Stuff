...
We want to solve the wave equation on a finite interval with Dirichlet boundary conditions:
...
So a function of $t$ is equal to a function of $x$. Clearly the only way this is possible is when these functions are both equal to a \textit{constant} value.
...
The $\lambda_n$'s are called \textbf{eigenvalues}, while the $X_n(x)$'s are called \textbf{eigenfunctions}, which are related to the concept of eigenvectors (we will explain this later). It should also be noted that, without loss of generality, we assume the constant $B = 1$, since we can always absorb it into $T_n(t)$. 
...
\[ (f,g) = \int_0^\ell f(x)\overline{g(x)} \ dx \]

and one can check that $(\partial_x^2f,g) = (f,\partial_x^2g)$. Another version of the spectral theorem says that there is an orthonormal basis 
