...
We will try and prove some formulas related to the heat equation, in the special case that we are dealing with a rod of infinite length. By doing this we are considering the case where $x \in \R$. We will also not consider 0 in the domain of $t$; the reason why will be obvious.

\begin{remark}
    The derivation we present here will seem somewhat random, as the tool used for deriving it, called the Fourier Transform, is not a core part of this course. The textbook presents another similar derivation that may yield better insights for some as to how we derive the following.
\end{remark}
...
\begin{proof}
    ...
    \begin{align*}
        \partial_tS - k\partial_{xx}S & = S\left(\frac{-1}{2t} + \frac{x^2}{4kt^2} + \frac{1}{2t} - \frac{x^2}{4kt^2}\right) \\
        & = 0
    \end{align*}
...
Before we proceed, recall the Gaussian Integral:

\[ \int_{-\infty}^\infty e^{-x^2} \ dx = \sqrt{\pi} \]

For each $t > 0$, we may consider 

\[ \int_{-\infty}^\infty S(x,t) \ dx \]

Informally, note that for fixed $t$, $S$ will go to 0 exponentially fast as $x \to \pm\infty$. Thus, this integral is well-defined. We write it as
...
We sometimes call this function $S$ the \textbf{heat kernel}. Let's try and understand what happens to the heat kernel as $t \to 0$. We first make some observations:
...
These observations help us understand what is going on: we start with a Gaussian-like curve with integral value 1. as $t$ gets smaller, this curve gets thinner and thinner, and $S$ is non-zero in a very small area around 0, while at 0 the value gets larger to preserve the area. Finally, at $t = 0$, we have a function that is 0 everywhere except at $x = 0$, where it has a value of infinity. This is shown below, where we show the graphs of $S$ for $t = 1, \dfrac{1}{2}, \dfrac{1}{10}$, and $t = \dfrac{1}{1000}$.

\begin{center}
    \includegraphics[width=0.7\textwidth]{Week 4/S at 1.png}
    \includegraphics[width=0.7\textwidth]{Week 4/S at half.png}
    \includegraphics[width=0.7\textwidth]{Week 4/S at tenth.png}
    \includegraphics[width=0.7\textwidth]{Week 4/S at thousandth.png}
\end{center}
...
We call $S$ an \textbf{approximation to the identity}. 

$S$ special because it is related to an important operation in PDE theory. For a continuous function $\phi(x)$ and $t > 0$, we can define
...
What does it mean when a solution to the IVP is \textit{unique}? Given $\phi(x)$, suppose 

\[ u_1(x,t) = u_2(x,t) \]

solve the IVP defined at the start of last section. It follows that

\[ (u_1)_t - k(u_1)_{xx} = (u_2)_t - k(u_2)_{xx} = 0 \]
\[ u_1(x,0) = u_2(x,0) = \phi(x) \]

so in general, is $u_1 = u_2$? No! 

Let's rephrase the above. Define $w = u_1 - u_2$. Now if

\[ w_t - kw_{xx} = 0 \]
\[ w(x,0) = 0 \]

does $w = 0$? Yes! But with more mild assumptions added onto it. 

Recall our solution from before

\[ u(x,t) = \frac{1}{\sqrt{4\pi kt}}\int_{-\infty}^\infty \exp\left(\frac{-(x,y)^2}{4kt}\right)\phi(y) \ dy \]

Let's assume that $\phi(x)$ is both continuous and \textit{compactly supported}, meaning it is nonzero only on some closed interval $[-R, R]$. What happens when we take $x$ to $\pm\infty$? Well, the exponential will go to 0, so the integral will also go to 0. Thus $u \to 0$
...
There are several differences between the heat and wave equations that are of importance to us. We're focusing on qualatiative properties here, not quantitative ones.

\begin{enumerate}
    \item The Speed of Propagation. 
    ...
    Contrast this with the heat equation, which actually has \textit{infinite} speed of propagation.
    ...
    The wave equation has \textit{singularity propagation}, meaning that while the wave will move, it will always look the same. Solutions to the wave equation are of the form
    ...
    If $g = 0$, then $u(x,0) = f(x)$, meaning $u(x,t) = f(x+ct)$. Thus, if $f$ is, say, $C^2$ but not $C^3$, so must 3. The differentiability of $u$ thus corresponds to the differentiability of $f,g$. 

    On the other hand, the heat equation \textit{immediately} smoothens out the initial data once $t > 0$. Solutions are of the form
    ...
    For the wave equation, $f,g$ will just translate in opposite directions as $t \to \infty$, so let's make it interesting. Consider the problem
    ...
    For the heat equation, assume that $\phi$ has compact support. Then we have that
