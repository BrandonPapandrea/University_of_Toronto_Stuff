...
We will now introduce and study the wave equation, a hyperbolic second order PDE. By the theorem from last week, we know hyperbolic PDEs can always be reduced to one that looks like the wave equation, so we study it in great detail.
...
Such an equation will correspond to a difference of squares,

\[ t^2 = c^2x^2 = (t-cx)(t+cx) \]

we can factor the partials in a similar way, yielding

\[ \left(\pardif{}{t} - c\pardif{}{x}\right)\left(\pardif{}{t} + c\pardif{}{x}\right)u = 0 \]
...
What does this look like? Recalling the simple transport equation, we see that as $t$ increases, $f$ will move to the left at speed $c$, while $g$ will move to the right at speed $-c$. This makes the $f$ and $g$ functions look like waves as time progresses. In the figure below, we set $f(x) = e^{-x^2}, g(x) = e^{-x^4}, c = 2.1$, and show the graphs of $f(x+ct) + g(x-ct)$ at $t = 0,0.5,1$, and 2:

\begin{center}
    \includegraphics[width=0.7\textwidth]{Week 3/Wave of Functions.png}
\end{center}

We say that the constant $c$ is the \textbf{speed of propagation}. If you recall from physics, we denote the speed of light by $c$; the fact that we use $c$ for both of these is not a coincidence. 
...
Recall that our general solution is $u(x,t) = f(x+ct) + g(x-ct)$, but now we have initial conditions corresponding to two equations:
...
To find the specific solution, we must solve for $f,g$. To do this, we first differentiate (3.1) to get
...
Notice that the constants make $f,g$ not unique. This is ok, since we are only looking for uniqueness of $u$, not $f$ or $g$. To check that $u$ is unique, we can plug $f,g$ into (3.1), which says
...
\begin{align*}
    u(x,t) & = f(x+ct) + g(x-ct) \\
    & = \frac{1}{2}\left[\varphi(x+ct) + \varphi(x-ct)\right] + \frac{1}{2c}\int_0^{x+ct} \psi(\tau) \ d\tau - \frac{1}{2c}\int_0^{x-ct} \psi(\tau) \ d\tau + A + B \intertext{as $t > 0$, $x - ct < x  +ct$. This, and that $A + B = 0$, we get} \\
    & = \frac{1}{2}\left[\varphi(x+ct) + \varphi(x-ct)\right] + \frac{1}{2c}\int_{x-ct}^{x+ct} \psi(\tau) \ d\tau
\end{align*}

This is indeed the unique continuous solution for this problem.
...
\begin{example}
    ...
    Graphically, this literally looks like a sine wave oscillating up and down as $t$ increases.
\end{example}
...
\begin{example}[The Plucked String Problem]
    ...
    \begin{center}
        \includegraphics[width=0.7\textwidth]{Week 3/Plucked String Solution.png}
    \end{center}
\end{example}
...
As a consequence of this, for an interval $I$ on which $\varphi, \psi$ vanish, if $u(x_0,t_0)$ is such that the corresponding path ight curve intersects $I$, then $u(x_0,t_0)$ = 0. Thus, if $\varphi, \psi$ are compactly supported, meaning they are nonzero in a bounded interval $[-A,A]$, then $u(x,t)$ is compactly supported at each time $t$. 
...
$E(t)$ is a well-defined function, since $u_x,u_t$ are compactly supported. We call this function the \textbf{energy} function of $u$.
...
We will return to the wave equation later on. For now, we move to studying the heat equation. Recall that it is given by
...
\begin{theorem}[The Maximum Principle]
    Suppose $u(x,t)$ solves the heat equation for $x \in [0,\ell]$ and $t \in [0,T]$. Then $u(x,t)$ attains a maximum, and it is attained on one of the lines 
    
    \[ \{x = 0\} \quad \{x = \ell\} \quad \{t = 0\} \]
\end{theorem}
...
\begin{theorem}
    There is at most one solution to the above IVP.
\end{theorem}

We present two proofs, one using the Maximum Principle, and another using energy:
...
\begin{proof}[Proof 2]
    ...
    where the $1/2$ is added for a nicer calculation. Intuitively, heat dissipates over time, and so $\dfrac{dE}{dt}$ should be less than or equal to 0. If so, then $E$ is decreasing in $t$, meaning that
    ...
    \begin{align*}
        \dif{E}{t} & = \dif{}{t}\frac{1}{2}\int_0^\ell (u(x,t))^2 \ dx \\
                   ...
                   & \overset{\text{IBP}}{=} -k\int_0^\ell (u_x)^2 \ dx + kuu_x\big|_0^\ell \\
                   & = -k\int_0^\ell (u_x)^2 \ dx \\
                   & \leq 0
    \end{align*}
