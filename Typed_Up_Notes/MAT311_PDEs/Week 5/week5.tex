...
Recall that $v(0,t) = 0$ is a (homogeneous) Dirichlet boundary condition that means there is no heat at one end of the rod. Let's also assume that $\phi(0) = 0$. 

The trick to solve this problem is to use \textbf{odd extension}. 
...
\begin{align*}
    ...
    & = \int_{-\infty}^\infty S(x-z,t)\phi_{odd}(y) \ dy \\
    & = -\int_{-\infty}^\infty S(x-z,t)\phi_{odd}(z) \ dz \\
    & = -u(x,t)
\end{align*}

as desired. Thus, $u(x,t)$ is also continuous when $t > 0$ and is odd in $x$. We also have that $u(0,t) = -u(0,t)$, so $u(0,t) = 0$. Graphically, this looks like some function which flips over the $x$-axis at $x = 0$: 
...
If we mimic the strategy for the heat equation by consider the odd extensions of $\phi, \psi$, then we will see that
...
To derive our solution, we use the following formula for differentiation of a definite integral:
